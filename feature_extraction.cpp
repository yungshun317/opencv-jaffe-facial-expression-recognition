...
void createDenseFeature(vector<KeyPoint> &keypoints, Mat image, float initFeatureScale=1.f, int featureScaleLevels=1, float featureScaleMul=0.1f, int initXyStep=6, int initImgBound=0, bool varyXyStepWithScale=true, bool varyImgBoundWithScale=false);
...
int main(int argc, char* argv[]) {
    ...
    // Clustering: compute feature distribution over k bins
    ...
    Mat labels, centers;
    int bin_size = 200;
    kmeans(rawFeatureData, bin_size, labels, TermCriteria(TermCriteria::COUNT+TermCriteria::EPS, 100, 1.0), 3, KMEANS_PP_CENTERS, centers);
    ...
    // [num_of_image * bin_size]
    Mat featureDataOverBins = Mat::zeros(num_of_image, bin_size, CV_32FC1);
    // for each image
    for (int i = 0 ; i < num_of_image; i++) {         
        Mat feature = Mat::zeros(1, bin_size, CV_32FC1);
        int num_of_image_features = features_vector[i].rows;
        // for each feature in cur image
        for (int j = 0; j < num_of_image_features; j++) {             
...
void createDenseFeature(vector<KeyPoint> &keypoints, Mat image, float initFeatureScale, int featureScaleLevels, float featureScaleMul, int initXyStep, int initImgBound, bool varyXyStepWithScale, bool varyImgBoundWithScale) {
